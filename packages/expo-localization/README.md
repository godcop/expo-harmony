...
```bash
npm install @expo-harmony/expo-localization expo-localization@55.0.15
```

本包适配 Expo SDK 55 的 `expo-localization`，原生模块通过 Expo Harmony 自动链接，不需要配置插件，也不需要申请权限。最低支持 HarmonyOS 5.0.1（API 13），宿主的 `compatibleSdkVersion` 也要满足这一要求。
...
返回 `Locale[]`，同步读取，至少一项。列表按系统偏好语言的顺序排列，每一项给出该语言自身的语言标签和货币，地区、数字格式与计量制取系统地区设置。
...
| `languageRegionCode`     | `string \| null`                    | 该语言自身的地区代码                |
